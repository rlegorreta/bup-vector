...
class VectorsCiaCia:
    ...
    def displayContext(self):
        ...
        MATCH (c:Compania)
        MATCH (c)-[r:SUBSIDIARIA|PROVEEDOR]-(t)
        WITH c, type(r) as type, COALESCE(r.tipo, "") as type_types, collect(t.nombre) as names
        WITH c, type +"<"+ type_types +">"+ ": " + reduce(s="", n IN names | s + n + ", ") as types
    ...
    def retrieveContext(self, question, k=3):
        ...
        MATCH (c:Compania)
        WITH c, gds.similarity.cosine(embedding, c.embedding) AS score
        ORDER BY score DESC
        ...
        MATCH (c)-[r:SUBSIDIARIA|PROVEEDOR]-(t)
        WITH c, similarCompanies, type(r) as type, COALESCE(r.tipo, "") as type_types, collect(t.nombre) as names
        WITH c, similarCompanies, type +"<"+ type_types +">"+": " + reduce(s="", n IN names | s + n + ", ") as types
