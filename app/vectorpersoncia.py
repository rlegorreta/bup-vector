...
# between Personas and Companies.
#
class VectorsPersonCia:
    ...
    def displayContext(self):
        ...
        MATCH (p:Persona)
        MATCH (p)-[r:TRABAJA]-(t)
        WITH p, type(r) as type, COALESCE(r.puesto, "") as position_types, collect(t.nombre) as names
        WITH p, type +"<"+ position_types +">"+ ": " + reduce(s="", n IN names | s + n + ", ") as types
    ...
    def generateTextEmbedding(self):
        ...
        'MATCH (p:Persona)
         ...
         SET p.embeddingCompanies = embedding',
    ...
    def retrieveContext(self, question, k=3):
        ...
        MATCH (p:Persona)
        WHERE COALESCE(p.embeddingCompanies, '') <> ''
        WITH p, gds.similarity.cosine(embedding, p.embeddingCompanies) AS score
        ORDER BY score DESC
        ...
        MATCH (p)-[r:TRABAJA]-(t)
        WITH p, similarPersons, type(r) as type, COALESCE(r.puesto, "") as position_types, collect(t.nombre) as names
        WITH p, similarPersons, type +"<"+ position_types +">"+": " + reduce(s="", n IN names | s + n + ", ") as types
